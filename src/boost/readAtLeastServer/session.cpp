...
void Session::handleHeadRead(const boost::system::error_code &ec, std::size_t bytesTransfreed, std::shared_ptr<Session> shared_self){
    if(!ec){
        if(bytesTransfreed < HEAD_LENGTH){
        ...
        std::bind(&Session::handleMsgRead, this, std::placeholders::_1, std::placeholders::_2, sharedSelf()));
...
void Session::handleMsgRead(const boost::system::error_code &ec, std::size_t bytesTransfreed, std::shared_ptr<Session> shared_self){
    if(!ec){
        ...
        std::cout << "recv data is: " << recvMsgNode->data << std::endl;
        send(recvMsgNode->data, recvMsgNode->totalLen);

        // again recv head
        recvHeadNode->clear();
        boost::asio::async_read(sock, asio::buffer(recvHeadNode->data, HEAD_LENGTH),
        std::bind(&Session::handleHeadRead, this, std::placeholders::_1, std::placeholders::_2, sharedSelf()));
